...
impl<'a> ReadFrom<'a, RowsDataV2<'a>> for [u8] {
    ...
    fn read_from(&'a self, offset: usize) -> Result<(usize, RowsDataV2<'a>)> {
        ...
        Ok((offset, RowsDataV2{table_id, flags, extra_data_len, payload}))
    }
}
...
impl<'a> ReadWithContext<'a, '_, RawRowsV2<'a>> for [u8] {
    ...
    fn read_with_ctx(&'a self, offset: usize, (extra_data_len, update): Self::Context) -> Result<(usize, RawRowsV2<'a>)> {
        ...
        let n_cols = n_cols.to_u32().ok_or_else(|| Error::ConstraintError(format!("invalid n_cols: {:?}", n_cols)))?;
    ...
    fn parse_rows(
        ...
        while !offset < self.rows_data.len() {
            ...
            let col_bm: Vec<u8> = bm1
                ...
                .map(|(b1, b2)| b1 ^ b2)
                .collect();
            let (os1, row): (_, Row) = self.rows_data.read_with_ctx(os1, (n_cols, &col_bm[..], col_metas))?;
            rows.push(row);
            offset = os1;
        }
    ...
    fn parse_update_rows(
        ...
        while offset < self.rows_data.len() {
            ...
            let before_col_bm: Vec<u8> = self
                ...
                .map(|(b1, b2)| b1 ^ b2)
                .collect();
            let (os1, before_row): (_, Row) = self.rows_data.read_with_ctx(os1, (n_cols, &before_col_bm[..], col_metas))?;
            // after row
            ...
            let after_col_bm: Vec<u8> = self
                ...
                .map(|(b1, b2)| b1 ^ b2)
                .collect();
            let (os1, after_row): (_, Row) = self.rows_data.read_with_ctx(os1, (n_cols, &after_col_bm[..], col_metas))?;
